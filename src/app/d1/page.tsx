...
const FALLBACK_EXCHANGE_RATE = 6.32; // Fallback rate if all APIs fail
            ...
            {!isPlanB && userData && (
              <div className="w-full mt-6">
                <button
                  ...
                  disabled={!account}
                >
                  {!account ? "กรุณาเชื่อมต่อกระเป๋า" : "ยืนยันเข้าร่วม Plan B"}
                </button>
              </div>
            )}
          </div>
      ...
      {showFirstConfirmationModal && (
        ...
        >
          <div className="p-6 bg-gray-900 rounded-lg border border-gray-700 max-w-md">
            <h3 className="text-xl font-bold mb-4 text-center">ยืนยันการเข้าร่วม Plan B</h3>
